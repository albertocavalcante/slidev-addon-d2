...
Use `` ```d2 {theme: 5} `` to set the theme:
...
Enables hand-drawn rendering style. Use `` ```d2 {sketch: true} ``:
...
The layout engine to use. `dagre` is the default and works well for most diagrams. `elk` handles complex nested diagrams better. Use `` ```d2 {layout: 'elk'} ``:
...
Padding in pixels around the diagram. Defaults to `100`. Use `` ```d2 {pad: 50} ``:
...
CSS scale factor applied to the SVG container. Useful for fitting large diagrams or making small ones larger. Use `` ```d2 {scale: 0.8} ``:
